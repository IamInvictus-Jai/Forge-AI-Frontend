...
const UploadZone: React.FC<UploadZoneProps> = ({ onFileSelect, status }) => {
  ...
  const validateAndUpload = (files: FileList | null) => {
    ...
    if (
      validTypes.some((type) => file.type.includes(type) || file.type === "")
    ) {
      onFileSelect(file);
    } else {
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div
        className={`mb-6 p-4 rounded-full transition-transform duration-300 ${isDragActive ? "scale-110" : "scale-100"} ${status === "error" ? "bg-red-500/10" : "bg-neutral-800"}`}
      >
        {status === "error" ? (
          <AlertCircle className="w-8 h-8 text-red-500" />
        ) : (
          <UploadCloud
            className={`w-8 h-8 ${isDragActive ? "text-orange-500" : "text-neutral-400"}`}
          />
        )}
      </div>
      ...
      <Button
        ...
        loadingText={
          status === "uploading"
            ? UI_COPY.states.uploading
            : UI_COPY.states.processing
        }
        variant="primary"
      ...
      <div className="space-y-1">
        {status === "error" ? (
          <p className="text-sm text-red-400 font-medium">
            {UI_COPY.states.error}
          </p>
        ) : (
          <>
            <p className="text-sm text-neutral-400 font-medium">
              {UI_COPY.upload.supportingText}
            </p>
            <p className="text-xs text-neutral-500">
              {UI_COPY.upload.privacyText}
            </p>
          </>
        )}
      </div>
