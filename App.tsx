...
import { ArrowRight, Sparkles, Scissors, Film, CheckCircle2, Zap } from 'lucide-react';
      ...
      <nav className="absolute top-0 left-0 w-full p-6 md:p-8 z-50 flex justify-between items-center pointer-events-none">
        <div className="flex items-center gap-3 pointer-events-auto group cursor-default">
          ...
          <div className="flex flex-col">
            <span className="font-bebas text-2xl md:text-3xl text-white tracking-wider leading-none mt-1">FORGE <span className="text-orange-500">AI</span></span>
          </div>
        </div>
      </nav>
        ...
        <div className="min-h-full w-full flex flex-col items-center justify-center py-20 md:py-24">
          <div className="w-full max-w-4xl px-6 flex flex-col items-center">
            ...
            <div className="text-center mb-10 md:mb-12 space-y-6 flex-shrink-0 max-w-3xl">
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-2 animate-slide-up backdrop-blur-md" style={{ animationDelay: '0.1s' }}>
                <Sparkles className="w-3 h-3 text-orange-400" />
                <span className="text-[10px] md:text-xs font-semibold tracking-wider text-neutral-300 uppercase font-display">Premium Video Editor</span>
              </div>
              
              {/* Headline: Bebas Neue, Regular */}
              {/* Gradient ONLY on "right in your browser" */}
              <h1 className="text-5xl md:text-7xl lg:text-8xl font-bebas tracking-wide text-white leading-[0.9] animate-slide-up drop-shadow-2xl uppercase" style={{ animationDelay: '0.2s' }}>
                 Edit videos Intelligently, <br />
                 <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-orange-400 to-orange-200">right in your browser</span>
              </h1>
              
              {/* Subheadline: Exo 2, Semibold */}
              <p className="text-sm md:text-lg font-exo font-semibold text-neutral-400 max-w-xl mx-auto animate-slide-up leading-relaxed" style={{ animationDelay: '0.3s' }}>
                {UI_COPY.landing.subtitle}
              </p>
            </div>

            {/* Dynamic Card Area */}
            <div className={`w-full max-w-2xl ${THEME_CLASSES.cardBg} rounded-2xl md:rounded-3xl p-1.5 shadow-2xl shadow-black/80 transition-all duration-500 animate-slide-up flex-shrink-0`} style={{ animationDelay: '0.4s' }}>
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-1">
                    <div className="flex flex-col gap-1 text-center md:text-left">
                      <div className="flex items-center gap-2 justify-center md:justify-start text-neutral-300 font-medium">
                        ...
                        <span className="text-sm font-display">Import Successful</span>
                      </div>
                      <p className="text-xs text-neutral-500 hidden md:block">{UI_COPY.preview.utilityText}</p>
                    </div>
            ...
            {/* Footer */}
            <footer className="mt-10 md:mt-16 text-center animate-fade-in flex-shrink-0" style={{ animationDelay: '0.6s' }}>
              <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 text-xs md:text-sm text-neutral-600 font-medium mb-3">
                <span className="flex items-center gap-1.5 hover:text-neutral-400 transition-colors cursor-default"><Film size={12} /> MP4</span>
                <span className="w-0.5 h-0.5 rounded-full bg-neutral-800"></span>
                <span className="hover:text-neutral-400 transition-colors cursor-default">MOV</span>
                <span className="w-0.5 h-0.5 rounded-full bg-neutral-800"></span>
                <span className="hover:text-neutral-400 transition-colors cursor-default">WebM</span>
                <span className="w-0.5 h-0.5 rounded-full bg-neutral-800 hidden md:block"></span>
                <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/5 text-[10px] tracking-wide text-neutral-500">4K READY</span>
              </div>
